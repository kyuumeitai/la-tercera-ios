...
NSMutableArray *listaCategorias8;
NSMutableArray *listaBeneficios8;

- (void)viewDidLoad {
    [super viewDidLoad];
    
    SingletonManager *singleton = [SingletonManager singletonManager];
    listaCategorias8 = [[NSMutableArray alloc] init];
    listaCategorias8 = singleton.categoryList;
    //NSLog(@"La lista de categorias es: %@",listaCategorias8.description);

    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"idCat == %d", 7];
    NSArray *filteredArray = [listaCategorias8 filteredArrayUsingPredicate:predicate];
    
    Category* firstFoundObject = nil;
    firstFoundObject =  filteredArray.count > 0 ? filteredArray.firstObject : nil;
    Category *categoria = firstFoundObject;
    
    listaBeneficios8 = [[NSMutableArray alloc] init];
    listaBeneficios8 = categoria.arrayBenefits;
    //NSLog(@"La lista de beneficios es: %@",listaBeneficios8.description);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row==0) {
        ...
        if (cell == nil)
        {
            ...
            Benefit *beneficio = [listaBeneficios8 objectAtIndex:0];
        ...
        Benefit *beneficio2 = [listaBeneficios8 objectAtIndex:indexPath.row];
        ...
        cell.imageCategoria.image = [[listaBeneficios8 objectAtIndex:0] imagenNormal];  //beneficio2.imagenNormal;
