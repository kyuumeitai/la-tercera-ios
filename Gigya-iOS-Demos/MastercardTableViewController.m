...
@interface MastercardTableViewController ()

@end

@implementation MastercardTableViewController
NSMutableArray *listaCategorias5;
NSMutableArray *listaBeneficios5;

- (void)viewDidLoad {
    [super viewDidLoad];
    
    SingletonManager *singleton = [SingletonManager singletonManager];
    listaCategorias5 = [[NSMutableArray alloc] init];
    listaCategorias5 = singleton.categoryList;
    //NSLog(@"La lista de categorias es: %@",listaCategorias5.description);
    

    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"idCat == %d", 2];
    NSArray *filteredArray = [listaCategorias5 filteredArrayUsingPredicate:predicate];
    
    Category* firstFoundObject = nil;
    firstFoundObject =  filteredArray.count > 0 ? filteredArray.firstObject : nil;
    Category *categoria = firstFoundObject;
    
    listaBeneficios5 = [[NSMutableArray alloc] init];
    listaBeneficios5 = categoria.arrayBenefits;
    //NSLog(@"La lista de beneficios es: %@",listaBeneficios5.description);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row==0) {
        ...
        if (cell == nil)
        {
            ...
            Benefit *beneficio = [listaBeneficios5 objectAtIndex:0];
        ...
        Benefit *beneficio2 = [listaBeneficios5 objectAtIndex:indexPath.row];
        ...
        cell.imageCategoria.image = [[listaBeneficios5 objectAtIndex:0] imagenNormal];  //beneficio2.imagenNormal;
